...
def run_screener():
    ...
    report_message = f"📊 *Multi-Timeframe SMC Screener ({len(symbols)} Coins)*\n\n"
    total_signals = 0
    ...
    for symbol in symbols:
        ...
        for tf in timeframes:
            try:
                ...
                # 1. Unmitigated Order Block Taps
                active_obs = ob_df[ob_df['MitigatedIndex'].isna()]
                if not active_obs.empty:
                    last_ob = active_obs.iloc[-1]
                    if last_ob['OB'] == 1 and (current_low <= last_ob['Top'] and current_high >= last_ob['Bottom']):
                        tf_signals.append("🟢 OB Tap")
                    elif last_ob['OB'] == -1 and (current_low <= last_ob['Top'] and current_high >= last_ob['Bottom']):
                        tf_signals.append("🔴 OB Tap")
                
                # 2. Unmitigated FVG Taps
                active_fvgs = fvg_df[fvg_df['MitigatedIndex'].isna()]
                if not active_fvgs.empty:
                    last_fvg = active_fvgs.iloc[-1]
                    if last_fvg['FVG'] == 1 and (current_low <= last_fvg['Top'] and current_high >= last_fvg['Bottom']):
                        tf_signals.append("🟢 FVG Tap")
                    elif last_fvg['FVG'] == -1 and (current_low <= last_fvg['Top'] and current_high >= last_fvg['Bottom']):
                        tf_signals.append("🔴 FVG Tap")
    ...
    if total_signals == 0:
        report_message += "_No active setups or zone taps found across timeframes._\n"
