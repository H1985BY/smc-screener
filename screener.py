    ...
    try:
        if len(message) > 4000:
            for x in range(0, len(message), 4000):
                bot.send_message(CHAT_ID, message[x:x+4000], parse_mode="Markdown")
        else:
...
def run_screener():
    ...
    if report_message == f"📊 *SMC Setup Screener Report ({len(symbols)} Coins)*\n\n":
        report_message += "No active SMC setups detected in this run."
