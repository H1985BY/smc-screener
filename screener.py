...
def run_screener():
    ...
    # Binance API 451 බ්ලොක් මඟහරවා ගැනීමට පොදු (public) හෝ විකල්ප എക്ස්චේන්ජ් ප්‍රොක්සි සෙටින්ග්ස් පාවිච්චි කිරීම
    exchange = ccxt.binance({
        'enableRateLimit': True,
        'options': {'defaultType': 'spot'}
    })
    
    # GitHub සර්වර්ස් වලින් Binance එකට බාධා වන විට ඊට විකල්පව Public API URL එකට මාරු වීම
    exchange.urls['api'] = {
    ...
    for symbol in symbols:
        try:
            ...
            if not np.isnan(last_fvg):
                ...
                report_message += f"• *{symbol}*: No FVG signal.\n"
