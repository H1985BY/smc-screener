...
def run_screener():
    ...
    report_message = f"📊 *SMC Imbalance & OB Tap Screener ({len(symbols)} Coins)*\n\n"
    total_signals = 0
    ...
    for symbol in symbols:
        ...
        for tf in timeframes:
            try:
                ...
                # Check recent 3 candles for touch/approach (allowing candle flexibility)
                recent_lows = df['low'].iloc[-3:]
                recent_highs = df['high'].iloc[-3:]
                
                tf_signals = []
                
                # 1. Unmitigated Order Blocks backed by Imbalance (FVG)
                active_obs = ob_df[ob_df['MitigatedIndex'].isna()]
                if not active_obs.empty:
                    for idx, ob in active_obs.iterrows():
                        # Check if any of the last 3 candles touched this OB zone
                        touched = False
                        for c_low, c_high in zip(recent_lows, recent_highs):
                            if c_low <= ob['Top'] and c_high >= ob['Bottom']:
                                touched = True
                                break
                        
                        if touched:
                            # Check if this OB is accompanied/backed by an FVG (Imbalance) nearby (within 3 candles)
                            fvg_near = fvg_df[(fvg_df.index >= idx) & (fvg_df.index <= idx + 3) & (fvg_df['FVG'].notna())]
                            if not fvg_near.empty:
                                ob_type = "🟢 Bullish OB + FVG Tap" if ob['OB'] == 1 else "🔴 Bearish OB + FVG Tap"
                                if ob_type not in tf_signals:
                                    tf_signals.append(ob_type)
                
                # 2. Unmitigated FVG Taps (Recent 3 candles window)
                active_fvgs = fvg_df[fvg_df['MitigatedIndex'].isna()]
                if not active_fvgs.empty:
                    for idx, fvg in active_fvgs.iterrows():
                        touched_fvg = False
                        for c_low, c_high in zip(recent_lows, recent_highs):
                            if c_low <= fvg['Top'] and c_high >= fvg['Bottom']:
                                touched_fvg = True
                                break
                        if touched_fvg:
                            fvg_type = "🟢 FVG Tap" if fvg['FVG'] == 1 else "🔴 FVG Tap"
                            if fvg_type not in tf_signals:
                                tf_signals.append(fvg_type)
    ...
    if total_signals == 0:
        report_message += "_No active OB+FVG taps found in recent candles across timeframes._\n"
