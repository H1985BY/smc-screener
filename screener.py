...
def run_screener():
    ...
    report_message = f"📊 *SMC Zone Tap Screener ({len(symbols)} Coins)*\n\n"
    total_signals = 0
    ...
    for symbol in symbols:
        ...
        for tf in timeframes:
            try:
                ...
                # Check recent 3 candles for touch/approach
                recent_lows = df['low'].iloc[-3:]
                recent_highs = df['high'].iloc[-3:]
                
                tf_signals = []
                
                # 1. Unmitigated Order Blocks Taps (Recent 3 candles window)
                    ...
                    for idx, ob in active_obs.iterrows():
                        ...
                        for c_low, c_high in zip(recent_lows, recent_highs):
                            if c_low <= ob['Top'] and c_high >= ob['Bottom']:
                                ...
                                break
                        if touched:
                            ob_type = "🟢 Bullish OB Tap" if ob['OB'] == 1 else "🔴 Bearish OB Tap"
    ...
    if total_signals == 0:
        report_message += "_No active zone taps found in recent candles across timeframes._\n"
