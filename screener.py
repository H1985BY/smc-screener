...
def find_first_mitigation_obs(klines_15m):
    ...
    for i in range(start_idx, total_candles - 3):
        ...
        if (ob_candle['close'] < ob_candle['open']) and (impulse_candle['close'] > impulse_candle['open']) and (impulse_candle['close'] > ob_candle['high']):
            if fvg_candle['low'] > ob_candle['high']:
                ...
                is_first_touch = (ob_candle['low'] <= last_candle['low'] <= ob_candle['high']) or \
                                 (ob_candle['low'] <= last_candle['close'] <= ob_candle['high'])
        ...
        if (ob_candle['close'] > ob_candle['open']) and (impulse_candle['close'] < impulse_candle['open']) and (impulse_candle['close'] < ob_candle['low']):
            if fvg_candle['high'] < ob_candle['low']:
                ...
                last_candle = klines_15m[-1]
                is_first_touch = (ob_candle['low'] <= last_candle['high'] <= ob_candle['high']) or \
                                 (ob_candle['low'] <= last_candle['close'] <= ob_candle['high'])
...
def check_5m_choch_confirmation(klines_5m, direction):
    if not klines_5m or len(klines_5m) < 15:
        return False
    
    recent_candles = klines_5m[-12:]
    if direction == "BULLISH":
        local_high = max(c['high'] for c in recent_candles[:-2])
        return klines_5m[-1]['close'] > local_high
    else:
        local_low = min(c['low'] for c in recent_candles[:-2])
        return klines_5m[-1]['close'] < local_low
...
def run_daytrading_screener():
    ...
    for symbol in TARGET_COINS:
        ...
        bullish_obs, bearish_obs = find_first_mitigation_obs(klines_15m)
        target_obs = bullish_obs if trend_direction == "BULLISH" else bearish_obs
        
        for ob in target_obs:
            has_confirmation = check_5m_choch_confirmation(klines_5m, trend_direction)
            ...
            signal_msg = (
                f"<b>⚡ SMC CONFIRMED SIGNAL (5m CHoCH)</b>\n\n"
                f"🎯 <b>Coin:</b> #{symbol} ({emoji})\n"
                f"🔥 <b>Trend:</b> {htf_status}\n"
                f"⏱️ <b>Setup:</b> 15m OB First Mitigation + 5m CHoCH\n\n"
