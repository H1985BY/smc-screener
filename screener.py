...
class smc:
    ...
    def ob(cls, ohlc: DataFrame, swing_highs_lows: DataFrame, close_mitigation: bool = False) -> Series:
        ...
        bottom = np.where(~npisnan(ob), ohlc["low"], np.nan)
...
def run_screener():
    ...
    report_message = f"📊 *Order Block (OB) Tap Screener ({len(symbols)} Coins)*\n\n"
    total_signals = 0
    ...
    for symbol in symbols:
        ...
        for tf in timeframes:
            try:
                ...
                # Check recent 3 candles for OB touch
                recent_lows = df['low'].iloc[-3:]
                recent_highs = df['high'].iloc[-3:]
                
                tf_signals = []
                
                # Unmitigated Order Blocks Taps (Recent 3 candles window)
    ...
    if total_signals == 0:
        report_message += "_No active OB taps found in recent candles across timeframes._\n"
