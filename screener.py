...
def run_screener():
    ...
    report_message = f"📊 *Order Block (OB) Active Tap Screener ({len(symbols)} Coins)*\n\n"
    total_signals = 0
    ...
    for symbol in symbols:
        ...
        for tf in timeframes:
            try:
                ...
                # Check current latest candle (iloc[-1])
                curr_low = df['low'].iloc[-1]
                curr_high = df['high'].iloc[-1]
                
                tf_signals = []
                
                # Unmitigated Order Blocks Check
                active_obs = ob_df[ob_df['MitigatedIndex'].isna()]
                if not active_obs.empty:
                    for idx, ob in active_obs.iterrows():
                        # Adding a 0.5% buffer zone so that if price is getting close, it triggers
                        buffer_top = ob['Top'] * 1.005
                        buffer_bottom = ob['Bottom'] * 0.995
                        
                        if curr_low <= buffer_top and curr_high >= buffer_bottom:
                            ob_type = "🟢 Bullish OB Touch/Near" if ob['OB'] == 1 else "🔴 Bearish OB Touch/Near"
    ...
    if total_signals == 0:
        report_message += "_No active OB touches or near zones found right now._\n"
